import {
    ...
    return (
      <React.Fragment>
       { /* <directionalLight
        ...
        <directionalLight position={[0, 5, -10]} intensity={2.4} />
  ...
  const Model = () => {
    const gltf = useGLTFLoader("/sceneBlackPanther.gltf", true);
    console.log(gltf);

    return <primitive position={[0,0,0]} object={gltf.scene} dispose={null} />;
  };
  ...
  const HtmlContent = () => {
    ...
    return (
      <mesh scale={[0.009,0.009,0.009]}  ref={ref} position={[0, -0.7, 0]}>
      <Model />
      </mesh>
      //{/*  <Html fullscreen>
